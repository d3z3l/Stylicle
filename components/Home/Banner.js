...
class Banner extends React.Component {
  ...
  componentDidMount = () => {
    if (this.props.service_filter_item != "") {
      this.setState({
        search_box_dropdown_selected_text: this.props.service_filter_item,
      });
    }
    if (this.props.service_filter_location_name != "") {
      this.setState({
        search_location_text: this.props.service_filter_location_name,
      });
    }
    this.getLocation();
    this.hendalregions();
        ...
        (position) => {
          fetch(
            "https://maps.googleapis.com/maps/api/geocode/json?latlng=" +
              ...
              "&key=AIzaSyDgw4m-jKG2UueegLrTdLfJfO4phMxS6wo"
          )
            .then((res) => res.json())
            .then((resjson) => {
              console.clear();
              let lastElement =
                resjson.results.pop().address_components[0].short_name;
  ...
  getLatlng = (id, text) => {
    ...
    CountriesHelper.GetlatlngByid(id)
      .then((resp) => {
        // console.log('ssssssssssssss');
        // console.log(resp.data.data.data);
        // console.clear();
        this.setState({
          search_location: resp.data.data.data,
          search_location_text: text,
        });
    ...
    return (
      <>
        ...
        <Modal
          ...
          height="auto"
          effect="fadeInUp"
          onClickAway={() => this.setState({ search_modal: false })}
              ...
              <div class="popup_box">
                <div style={{ width: "100%", position: "relative" }}>
                  ...
                  <select
                    class="form-control"
                    onChange={(text) => [
                      this.setState({ selected_country: text.target.value }),
                    ]}
                  >
                    {this.state.countries.map((val) => (
                      <option
                        selected={val.code == this.state.selected_country}
                        value={val.code}
                      >
                        {val.name}
                      </option>
        ...
        <section id="banner_wrepp">
          ...
          <div class="banner_content">
            ...
            <form class="">
                ...
                <input
                  ...
                  onChange={(e) => {
                    this.search(e.target.value),
                      this.setState({
                        search_box_dropdown: true,
                        search_box_dropdown_selected_text: e.target.value,
                      }),
                      e.target.value == ""
                        ? (this.props._service_filter_item(""),
                          this.props._service_filter(""))
                        : null;
                  }}
                />
              </div>
              ...
              <button
                type="button"
                onClick={() => this.hendalSearchService()}
                class="btn-default"
              >
                Search
              </button>
            ...
            <div class="bg-white search-r">
              {this.state.search_box_dropdown
                ? this.state.result.map((val, index) => (
                    ...
                    <li
                      onClick={() => {
                        this.props._service_filter(val._id),
                          this.props._service_filter_item(val.title),
                          this.setState({
                            search_box_dropdown: false,
                            search_box_dropdown_selected_text: val.title,
                          });
                      }}
                    >
                      {val.title}
