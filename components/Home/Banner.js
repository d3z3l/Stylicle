      ...
      for (let i = 0; i < resp.data.data.services.length; i++) {
        ...
        services.push(
          <Link  href="/marketpalce/MarketplaceCategories" >
            <li onClick={()=>{ this.props._service_filter(element._id) }} >
          ...
          <div class="banner_content banner_content_popup">
            ...
            <div style={{ width: "100%",position:'relative'}} >
                <span
                  ...
                  style={{ width: "16px", height: "16px",left:'16px' }}
                >
                  <FontAwesomeIcon
                  onClick={()=>this.setState({search_modal:false})}
                    style={{
                      color: '#00000',
                      fontSize:'20px',
                      cursor: "pointer",
              ...
              <div style={{ width: "100%",position:'relative'}} >
                ...
                <select
                class="form-control"
                onChange={(text) => [this.setState({ selected_country: text.target.value })]}
                >
                  {
                    this.state.countries.map((val)=>(
                      <option selected={val.code==this.state.selected_country} value={val.code}  >{val.name}</option>
                    ))
                  }
                </select>
              </div>
              ...
              <div class="d-flex justify-content-center py-3" >
                <button onClick={()=>this.setState({search_modal:false})} style={{padding:' 13px 74px'}} class="btn-default">Select</button>
              </div>
          ...
          <div class="banner_content">
            ...
            <div style={{ width: "100%",position:'relative'}} >
              ...
              <input
                type="search"
                class="form-control"
                placeholder="Search Your Servicesssss"
                value={this.state.search_box_dropdown_selected_text}
                onChange={(e)=> {this.search(e.target.value),this.setState({search_box_dropdown:true,search_box_dropdown_selected_text:e.target.value}),e.target.value==''?(this.props._service_filter_item(''),this.props._service_filter('')):null}}
              />
            </div>
            <div onClick={()=>this.setState({search_modal:true})} style={{ width: "100%",position:'relative'}} >
              <span
                class="search-bar-icon"
                style={{ width: "16px", height: "16px",left:'16px' }}
              >
              ...
              <input
                value={this.state.search_location_text}
                apiKey={'AIzaSyDgw4m-jKG2UueegLrTdLfJfO4phMxS6wo'}
            ...
            <button type="button" onClick={()=>this.hendalSearchService()}  class="btn-default">Search</button>
            </form>
            <div class="bg-white search-r">
              {
                this.state.search_box_dropdown?
                  this.state.result.map((val,index)=>(
                    // <Link  href="/marketpalce/MarketplaceCategories" >
                      <li onClick={()=> {this.props._service_filter(val._id),this.props._service_filter_item(val.title),this.setState({search_box_dropdown:false,search_box_dropdown_selected_text:val.title}) }} >{val.title}</li>
                    // </Link>
                  ))
                :null
              }
