...
class Categories extends React.Component {
  ...
  hendalServicesTimeSlote = async (id) => {
    ...
    await AssignServicesHelper.Get(id).then((resp) => {
      for (
        let i = 0;
        i < resp.data.data.assignservices[0].service_time_slot.length;
        i++
      ) {
        ...
        servicestimeslotes.push([
          ...
          moment(
            new Date(
              (element.start_time + new Date().getTimezoneOffset() * 60) * 1000
            )
          ).format("LT"),
          moment(
            new Date(
              (element.end_time + new Date().getTimezoneOffset() * 60) * 1000
            )
          ).format("LT"),
          days[element.day],
  ...
  update = () => {
    ...
    ServicesTimeSlotsHelper.Update(this.state.update_slot_id, data).then(
      (resp) => {
        this.setState({ message: "" });
      ...
      <>
        ...
        {this.state.show ? (
          <SweetAlert
            ...
            title="Are you sure?"
            onConfirm={() =>
              this.hendalDeleteSlot(this.state.delete_service_id)
            }
                  ...
                  <form>
                    <div class="mb-3">
                      ...
                      <select
                        onChange={(text) => [
                          this.setState({
                            assignservice_id: text.target.value,
                          }),
                        ]}
                        class="form-control selector"
                      >
                        <option
                          selected={this.state.assignservice_id == ""}
                          value=""
                          disabled
                        >
                          Select Any Services
                        ...
                        {this.state.assignservices.map((val, index) =>
                          val.services ? (
                            <option
                              selected={this.state.assignservice_id == val._id}
                              value={val._id}
                            >
                              {val.services.title}
                            </option>
                      ...
                      <Datetime
                        ...
                        onChange={(time) => {
                          ...
                          this.state.from =
                            time1 - time2 + Math.sqrt(zone * zone) * 60;
                          this.setState({
                            from: time1 - time2 + Math.sqrt(zone * zone) * 60,
                          });
                          // this.hendalfielda()
                        }}
                      />
                      ...
                      <Datetime
                        ...
                        onChange={(time) => {
                          ...
                          this.state.to =
                            time1 - time2 + Math.sqrt(zone * zone) * 60;
                          this.setState({
                            to: time1 - time2 + Math.sqrt(zone * zone) * 60,
                          });
                          // this.hendalfielda()
                        }}
                      />
                    ...
                    <div class="mb-3">
                      ...
                      >
                        ...
                        <option selected={this.state.day == "1"} value="1">
                          Monday
                        </option>
                        <option selected={this.state.day == "2"} value="2">
                          Tuesday
                        </option>
                        <option selected={this.state.day == "3"} value="3">
                          Wednesday
                        </option>
                        <option selected={this.state.day == "4"} value="4">
                          Thursday
                        </option>
                        <option selected={this.state.day == "5"} value="5">
                          Friday
                        </option>
                        <option selected={this.state.day == "6"} value="6">
                          Saturday
                        </option>
                        <option selected={this.state.day == "7"} value="7">
                          Sunday
                        </option>
                      </select>
                      <p class="Primery_color">{this.state.message}</p>
                ...
                <div class="col-md-4">
                  <div class="assign_box">
                    ...
                    <form>
                        ...
                        <>
                          <div class="mb-3">
                            ...
                            <Datetime
                              ...
                              onChange={(time) => {
                                var time1 =
                                  new Date(time.format()).getTime() / 1000;
                                var time2 =
                                  new Date(
                      ...
                      <a
                        onClick={this.hendalCreateServicesTimeSlote}
                        href="#."
                        class="btn button bg-blue-700 btn-info"
                      >
                        Assign new Service
                  ...
                  <div class="card p-3 uk-box-shadow-medium">
                    <div class="col-md-8">
                      <div class="mb-3">
                        ...
                        <select
                          onChange={(text) =>
                            this.hendalServicesTimeSlote(text.target.value)
                          }
                          class="form-control"
                        >
                          <option
                            selected={this.state.servicestimeslote_id == ""}
                            value=""
                            disabled
                          >
                            Select Any Services
