...
class ViewSeller extends React.Component {
  ...
  hendalOrders = () => {
    ...
    AuthHelper.Update_temp(data).then((resp)=>{
		var method=''
		...
		TransactionHelper.create_admin_temp(data2).then((resp)=>{
			this.setState({reservations:[]})
			cookie.save('Tokken', cookie.load('Tokken_temp') , { path: '/' })
